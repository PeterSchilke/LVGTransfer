...
//
//  Time-stamp: <2006-01-03 16:38:18 schilke> 
//
#include <iostream>
#include <iomanip>
#include "Mols.h"
...
// header
	...
	{
	    plotFile << thisCloud.Tkin() << " ";          //1
	    plotFile << thisCloud.nH2() << " ";           //2
	    plotFile << N_tot << " ";                     //3
	    plotFile << thisCloud.Tback() << " ";         //4
	    plotFile << thisCloud.tau100mic() << " ";     //5
	    plotFile << thisCloud.Wback() << " ";         //6
	    plotFile << thisCloud.Te() << " ";            //7
	    plotFile << thisCloud.EM() << " ";            //8
	    plotFile << thisCloud.WHII() << " ";          //9
	}
...
//			      - RJ(thisCloud.Tdust(),freq)*(1-exp(-tau_d_inf))
		...
		if (Convergence == 1)
		    // only write to .plt file if converged
		    {
			plotFile << tauBack << " ";  //10  16  22
			plotFile << T_lBack << " ";  //11  17  23
			plotFile << tauHII << " ";   //12  18  24
			plotFile << T_lHII << " ";   //13  19  25
			plotFile << tau << " ";      //14  20  26
			plotFile << T_l << " ";      //15  21  27
		    }
	    }
	}
    }
