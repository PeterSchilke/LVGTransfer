//
// Atoms.cc	-- Peter Schilke Wed Feb 12 1997
//
//  Time-stamp: <2006-11-20 15:41:36 schilke> 
//
...
#include "Mols.h"
//atomicCarbon
atomicCarbon::atomicCarbon(int size, string MolFile, string ColFile,
...
// check if number of read energy levels is smaller than requested...    
    ...
    for (i=1; i<size; i++)
	{
	    for (f=0;f<i;f++)
		{
		    ...
		    if (Ji == 1 && Jf == 0)
#ifdef BLITZ
			Einstein_A(i,f) = 7.93e-8;
#else
			Einstein_A[i][f] = 7.93e-8;
#endif
		    else if (Ji == 2 && Jf == 0)
#ifdef BLITZ
			Einstein_A(i,f) = 1.71e-14;
#else
			Einstein_A[i][f] = 1.71e-14;
#endif
		    else if (Ji == 2 && Jf == 1)
#ifdef BLITZ
			Einstein_A(i,f) = 2.65e-7; 
#else
			Einstein_A[i][f] = 2.65e-7; 
#endif
		    else 
