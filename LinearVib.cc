...
//
//  Time-stamp: <2006-11-27 17:51:12 schilke> 
//
...
#include "Mols.h"
//LinearVib
LinearVib::LinearVib(int size, string MolFile, string ColFile,
...
// check if number of read energy levels is smaller than requested...    
    if (i < size) 
	{
	    ...
	    cout << "Using fewer levels than requested: "
		 << i-1 << " instead of " << Number_Of_Levels << endl;
	}
...
#endif
		    int deltaJ = Ji-Jf;
		    if (abs(deltaJ) == 1)    // deltaJ = 1 selection rule 
			{
			    if (vi == vf)
				{
#ifdef BLITZ
				    Einstein_A(i,f) = 64 * pow(Pi,4) * pow(freq,3)/
					(3*h_Planck*pow(c_Light, 3))*mu[vi]*mu[vi];
#else
				    Einstein_A[i][f] = 64 * pow(Pi,4) * pow(freq,3)/
					(3*h_Planck*pow(c_Light, 3))*mu[vi]*mu[vi];
#endif
...
//	    cout << Ji << " " << Jf << " ";
	    ...
	    index_i = index_v_J[0][Ji];
	    index_f = index_v_J[0][Jf];
//	    cout << "Ji " << Ji << " " << index_i << " Jf " << Jf << " " << index_f << " " << Number_Of_Levels << endl;
...
//		    cout << endl;
    ...
    if (! plotFile)  // open failed 
	{
	    ...
	    {
		plotFile << "! Tkin n(H2) N(Mol) T(back) tau(100mic) W(back) T(e) EM W(HII) "  
			 << "tau(back)[1-0] T_l(back)[1-0] tau(HII)[1-0] T_l(HII)[1-0] tau(1-0) T_l(1-0) etc."
