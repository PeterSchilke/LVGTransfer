...
//
//  Time-stamp: <2005-12-29 18:26:29 schilke> 
//
...
#include "Mols.h"
//LinearVib
LinearVibl::LinearVibl(int size, string MolFile, string ColFile,
...
// now read in the molecular data
    ...
    do {
	inMolFile >> v_Of_Level[i] >> J_Of_Level[i] >> l_Of_Level[i] >> Energy_Of_Level[i];
	g_Of_Level[i] = 2*J_Of_Level[i]+1;
	index_v_J_l[ v_Of_Level[i] ][ J_Of_Level[i] ][ l_Of_Level[i] ] = i;
	n_Of_Level[i] = 0;
	    ...
	    for (f=0;f<i;f++)
		{
		    ...
		    if (abs(deltaJ) == 1)    // deltaJ = 1 selection rule 
			{
			    if (vi == vf)
				{
				    ...
				    if (deltaJ == 1 && deltal == 0)
					{
#ifdef BLITZ
					     Einstein_A(i,f) *= (Ji*Ji-li*li)/(Ji*(2.*Ji+1.));
#else
					     Einstein_A[i][f] *= (Ji*Ji-li*li)/(Ji*(2.*Ji+1.));
#endif
//					     cout << i << " " << Ji << ": " << Einstein_A(i,f) << endl;
					}
				    else if (deltaJ == 0 && deltal == 2) 
					{
#ifdef BLITZ					    
					    Einstein_A(i,f) *=
						li*li/((Ji+1)*(2.*Ji+1.));
...
//	    cout << Ji << " " << Jf << " ";
	    ...
	    index_i = index_v_J_l[0][Ji][0];
	    index_f = index_v_J_l[0][Jf][0];
