...
//
//  Time-stamp: <2006-11-27 19:10:40 schilke> 
//
#include <sstream>
#include <string>
#include "Mols.h"
...
void Transitions::DoCalc(Cloud &thisCloud,char sl,char *bo)
{
    char bo_save = *bo;
...
//		    cout << "DoCalc - entering PrintOut\n";
    ...
    else 
	{
		...
		Nmol,NmolMin,NmolFac,NmolMax,Tdust,NH2,delta_v,Tback,TbackMin,TbackStep,TbackMax,
		beta_back,tau100mic,tau100micMin,tau100micFac,tau100micMax,Wback,
		Te,EM,WHII,T_cosmic;
	    MinMaxInput(TkinMin,TkinStep,TkinMax,nH2Min,nH2Fac,nH2Max,XeMin,XeFac,XeMax,
			NmolMin,NmolFac,NmolMax,Tdust,NH2,delta_v,TbackMin,TbackStep,TbackMax,
			beta_back,tau100micMin,tau100micFac,tau100micMax,Wback,
			Te,EM,WHII,T_cosmic);
//			    cout << endl;
...
//						     // cout << "setting cloud" << endl;
							    *bo = bo_save;
							    ...
							    // printing out
							    this->PrintOut(thisCloud);
							    tau100mic *= tau100micFac;
							}
						    while (tau100mic < tau100micMax*tau100micFac);
						    tau100mic = tau100micMin;
						    Tback += TbackStep;
						}
					    while (Tback < TbackMax+TbackStep);
					    Tback = TbackMin;
					    Nmol = Nmol *= NmolFac;
					}
